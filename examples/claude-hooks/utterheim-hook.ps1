...
# the utterheim speak endpoint at http://127.0.0.1:7223/speak.
#
# Per ADR 0003 the wire format is the published language. This script is a
# convenience shim — `curl` or the bundled `utterheim-speak.exe` CLI work
# equally well.
#
# Voice routing convention (caller-side): each shell sets UTTERHEIM_VOICE
...
#
# Usage:
#   .\utterheim-hook.ps1 -Text "task done"
#   .\utterheim-hook.ps1 -Text "input required" -Voice marius
#   .\utterheim-hook.ps1 -Text "task done" -Silent
#
# Exit codes:
#   0  speak request accepted (HTTP 202)
#   2  HTTP error from utterheim (validation / server error)
#   3  cannot reach utterheim (sidecar not running, port in use elsewhere, etc.)
#
...
param(
    ...
    # Voice id. Defaults to $env:UTTERHEIM_VOICE, then "alba" (a pocket-tts
    # built-in that ships with utterheim out of the box).
    [string]$Voice,

    # Override the endpoint base URL. Defaults to $env:UTTERHEIM_ENDPOINT,
...
if ([string]::IsNullOrWhiteSpace($Voice)) {
    if ($env:UTTERHEIM_VOICE) { $Voice = $env:UTTERHEIM_VOICE }
    else { $Voice = 'alba' }
}
...
try {
    ...
    # Utterheim returns 202 Accepted with {requestId, queuePosition}.
    if ($response.StatusCode -ge 200 -and $response.StatusCode -lt 300) {
        exit 0
    }

    Write-Error "utterheim-hook: HTTP $($response.StatusCode) from $url"
    ...
    if ($Silent) { exit 0 }
    if ($status) {
        Write-Error "utterheim-hook: HTTP $status from $url — $($_.Exception.Message)"
        exit 2
    }
    Write-Error "utterheim-hook: cannot reach $url — is utterheim running? ($($_.Exception.Message))"
    exit 3
}
catch {
    if ($Silent) { exit 0 }
    Write-Error "utterheim-hook: $($_.Exception.Message)"
    exit 3
}
