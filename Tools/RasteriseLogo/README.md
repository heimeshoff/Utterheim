...
One-shot helper that turns `assets/branding/utterheim-logo.svg` into the PNG +
...
Only when the source SVG (`assets/branding/utterheim-logo.svg`) changes. The
...
- `utterheim-logo-{16,24,32,48,64,128,256,512}.png`
- `utterheim.ico` (multi-resolution: 16/24/32/48/64/128/256, all PNG-compressed)

## Why it isn't in `utterheim.sln`

Keeping it standalone means `dotnet build utterheim.sln` doesn't pull
