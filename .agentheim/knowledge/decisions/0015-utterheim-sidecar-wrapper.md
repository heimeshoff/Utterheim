---
id: 0015
title: Utterheim-owned Python sidecar wrapper for /export-voice
scope: global
status: accepted
date: 2026-05-04
supersedes: []
...
# ADR 0015: Utterheim-owned Python sidecar wrapper for /export-voice
...
the right shape — but utterheim does not own
...
Ship a small **utterheim-owned Python wrapper** module at
`runtime\python\Lib\site-packages\utterheim_sidecar\` that:
...
`utterheim_sidecar` package into site-packages from a bundled file
in the install, or pip-installs it from a wheel co-located with the
utterheim release. The choice is ADR-incidental; copy-from-bundle
...
- **Natural extension point.** Future utterheim-specific endpoints
...
- **Pinned import to pocket_tts internals.** The wrapper does
  ...
  pocket-tts version is incompatible with utterheim; update
  utterheim or downgrade pocket-tts.").
- **Bootstrapper now ships a Python artefact utterheim owns.**
...
- The wrapper module is a candidate for upstream contribution to
  ...
  v1, owning it keeps utterheim in control of the surface.
...
- **Patch `pocket_tts/main.py` in place** — rejected: fights
  ...
  utterheim to a specific pocket-tts release in a way that's
  invisible from outside the runtime. The wrapper module is
  utterheim-owned, lives outside the pocket_tts package, and
...
- **`serve` command loads the model itself, not via a `_initialize_model`
  ...
  `utterheim_sidecar.main:serve` and assign the result to
