...
Utterheim needs WhisperHeim's audio capture, hotkeys, settings/path, startup, and tray-shell infrastructure. None of those services are currently library-shaped — they live in `WhisperHeim.Services.*` with WhisperHeim-specific names. Choosing the reuse form (shared lib / copy-and-modify / submodule) up front avoids a half-extracted limbo state during walking-skeleton work.

## What

**Copy-and-modify for v1.** Copy the relevant source files into utterheim's tree, rewrite namespaces to `Utterheim.Services.*`, retain `// Adapted from WhisperHeim/<path> @ <commit>` headers and a CHANGELOG entry per copied file. After both apps ship v1 and surfaces stabilise, extract truly-shared services into `Heimeshoff.Audio` / `Heimeshoff.Hotkeys` shared projects.
...
- Settings/path: `DataPathService`, `SettingsService` (rename WhisperHeim → Utterheim in path strings)
- Startup: `StartupService` (run-at-login)

## Acceptance criteria

- [ ] ADR 0006 committed at `.agentheim/knowledge/decisions/0006-whisperheim-reuse-copy-and-modify.md` with `scope: global`.
...
Shared-library-up-front was ruled out: WhisperHeim's services are not yet library-shaped; extracting them is its own project; would block utterheim's walking skeleton.

Open follow-up: a small script that diffs copied files against WhisperHeim originals to flag drift. Not v1.

## Outcome

Decision recorded: **copy-and-modify** WhisperHeim infrastructure into utterheim's tree for v1, with provenance headers and CHANGELOG entries; revisit shared-library extraction post-v1. Unblocks `main-009` (actual file copy).

- ADR: `.agentheim/knowledge/decisions/0006-whisperheim-reuse-copy-and-modify.md`

Full ADR draft (dropped into `0006-whisperheim-reuse-copy-and-modify.md`):
