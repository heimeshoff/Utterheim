"""
Mockingbird sidecar wrapper around pocket-tts (ADR 0015).
...
can swap one launch arg and otherwise behave identically:
    ...
    python -m mockingbird_sidecar serve --host 127.0.0.1 --port 0
...
try:
    ...
    raise ImportError(
        "mockingbird_sidecar requires pocket-tts 2.x with the documented "
        "module layout (web_app exported from pocket_tts.main). "
        "Update mockingbird or pin pocket-tts to a compatible 2.x release."
    ) from exc
...
# not prevent `python -m mockingbird_sidecar serve` from at least logging
# the problem.

logger = logging.getLogger("mockingbird_sidecar")
...
async def export_voice(
    ...
    tmp_dir = tempfile.mkdtemp(prefix="mockingbird_clone_")
...
async def tts_with_state(
    ...
    tmp_dir = tempfile.mkdtemp(prefix="mockingbird_speak_")
...
# SidecarHost can swap "pocket_tts" for "mockingbird_sidecar" with no other
...
def serve(
    ...
    """Start the mockingbird-wrapped pocket-tts FastAPI app.
    ...
    logger.info("mockingbird_sidecar: loading pocket-tts model %s", load_kwargs or "(defaults)")
    try:
