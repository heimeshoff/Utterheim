...
//
// Mockingbird-specific divergences vs WhisperHeim:
...
//     mockingbird only uses the temp WAV path; the persistent location belongs
...
public sealed class HighQualityLoopbackService : IHighQualityLoopbackService
{
    ...
    public void StartCapture(int deviceIndex = -1)
    {
        ...
        var tempDir = Path.Combine(Path.GetTempPath(), "Mockingbird");
