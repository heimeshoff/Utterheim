// Utterheim-Speak CLI — thin wrapper over POST /speak.
// Per ADR 0003: Claude hooks should be able to invoke
//   utterheim-speak --voice alba "task done"
// without remembering curl flags.
using System.Net.Http.Json;
using System.Text.Json;

namespace Utterheim.Cli;
...
internal static class Program
{
    ...
    public static async Task<int> Main(string[] args)
    {
        ...
        var endpoint = Environment.GetEnvironmentVariable("UTTERHEIM_ENDPOINT") ?? DefaultEndpoint;
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                await Console.Error.WriteLineAsync($"utterheim-speak: HTTP {(int)response.StatusCode} {response.ReasonPhrase}");
        ...
        {
            await Console.Error.WriteLineAsync($"utterheim-speak: cannot reach {url} — is utterheim running? ({ex.Message})");
            return 3;
        }
    }
    ...
    {
        Console.WriteLine("Usage: utterheim-speak [--voice <id>] \"<text>\"");
        Console.WriteLine();
        ...
        Console.WriteLine("  UTTERHEIM_ENDPOINT  Override base URL (default: http://127.0.0.1:7223)");
    }
}
