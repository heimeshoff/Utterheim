...
using Utterheim.Services.Hotkey;
using Utterheim.Services.Http;
using Utterheim.Services.Navigation;
...
using Utterheim.ViewModels;
using Utterheim.ViewModels.Pages;
using Utterheim.Views;
using Utterheim.Views.Pages;
using Serilog;
...
public static class EntryPoint
{
    ...
    public static int Main(string[] args)
    {
        ...
        var logFilePath = Path.Combine(pathBootstrap.LogsPath, "utterheim-.log");
        ...
        {
            Log.Fatal(ex, "Utterheim host crashed.");
            return 1;
        }
        finally
        {
    ...
    private static int Run(string[] args, DataPathService pathBootstrap)
    {
        ...
        var config = new ConfigurationBuilder()
            ...
            .AddEnvironmentVariables(prefix: "UTTERHEIM_")
            .AddCommandLine(args)
            .Build();

        var httpHost = config["Utterheim:Http:Host"] ?? "127.0.0.1";
        var httpPort = int.TryParse(config["Utterheim:Http:Port"], out var p) ? p : 7223;
        var hotkeyWindowMs = int.TryParse(config["Utterheim:Hotkey:DoubleTapWindowMs"], out var hk) ? hk : 400;
        ...
        var hostBuilder = Host.CreateDefaultBuilder(args)
            ...
            .ConfigureServices((ctx, services) =>
            {
                ...
                // an env flag (UTTERHEIM_USE_STUB_ENGINE=1) for offline/CI testing per
                ...
                var useStub = string.Equals(
                    Environment.GetEnvironmentVariable("UTTERHEIM_USE_STUB_ENGINE"),
                ...
                // toggle reads/writes HKCU\…\Run\Utterheim directly.
