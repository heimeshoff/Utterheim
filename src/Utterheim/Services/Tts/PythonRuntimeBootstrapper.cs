...
public sealed class PythonRuntimeBootstrapper
{
    ...
    /// (<see cref="PocketTtsActuallyInstalled"/>, <see cref="UtterheimSidecarActuallyInstalled"/>,
    ...
    public async Task BootstrapAsync(IProgress<BootstrapProgress> progress, CancellationToken ct)
    {
        ...
        if (!state.PythonExtracted || !File.Exists(PythonExePath))
        {
            ...
            if (state.PipInstalled || state.PocketTtsInstalled || state.UtterheimSidecarInstalled || state.RuntimeReady)
            {
                _logger.LogInformation(
                    "Re-extracting Python runtime — resetting downstream bootstrap flags (PipInstalled, PocketTtsInstalled, UtterheimSidecarInstalled, RuntimeReady).");
                state.PipInstalled = false;
                state.PocketTtsInstalled = false;
                state.UtterheimSidecarInstalled = false;
        ...
        if (!state.PipInstalled || !PipExists())
        {
            ...
            // is also gone — same defensive reset as step 1. The utterheim_sidecar
            // copy is independent of pip but lives in the same site-packages tree, so
            // it's also gone if site-packages was wiped.
            if (state.PocketTtsInstalled || state.UtterheimSidecarInstalled || state.RuntimeReady)
            {
                _logger.LogInformation(
                    "Re-installing pip — resetting downstream flags (PocketTtsInstalled, UtterheimSidecarInstalled, RuntimeReady).");
                state.PocketTtsInstalled = false;
                state.UtterheimSidecarInstalled = false;
                state.RuntimeReady = false;
            }
        ...
        if (!state.PocketTtsInstalled || !PocketTtsActuallyInstalled())
        {
            if (state.PocketTtsInstalled && !PocketTtsActuallyInstalled())
            {
                ...
                state.PocketTtsInstalled = false;
                // utterheim_sidecar lives in the same site-packages tree as pocket-tts;
        ...
        // Step 3b — install the utterheim_sidecar wrapper (ADR 0015) into the
        // bootstrapped runtime's site-packages. The wrapper itself is a tiny
        // pure-Python package shipped next to utterheim.exe; it has no pip
        ...
        if (!state.UtterheimSidecarInstalled
            || !UtterheimSidecarActuallyInstalled()
            || !BundledSidecarMatchesInstalled())
        {
            progress.Report(new BootstrapProgress(
                BootstrapStep.InstallPocketTts, 0.95, "Installing utterheim sidecar wrapper…"));
            InstallUtterheimSidecar();
            state.UtterheimSidecarInstalled = true;
            state.RuntimeReady = false; // force smoke test re-run on first install
            SaveState(state);
            _logger.LogInformation("utterheim_sidecar wrapper installed.");
        }

        // Step 4 — smoke test: import both pocket_tts and utterheim_sidecar.
        ...
        await RunPythonInlineAsync(
            "import pocket_tts; import utterheim_sidecar; print('pocket_tts + utterheim_sidecar ok')",
            ct,
    ...
    /// <summary>
    /// Copy the bundled utterheim_sidecar package from the install folder
    /// into the bootstrapped Python runtime's site-packages. Idempotent —
    /// existing files are overwritten so a utterheim upgrade refreshes the
    /// wrapper. Per ADR 0015 the bundled source lives next to the .exe under
    /// <c>PythonSidecar/utterheim_sidecar/</c>.
    /// </summary>
    private void InstallUtterheimSidecar()
    {
        var sourceRoot = LocateBundledSidecarRoot();
        if (sourceRoot is null)
        {
            throw new InvalidOperationException(
                "Could not locate bundled utterheim_sidecar Python package. " +
                "Expected at <appBaseDirectory>/PythonSidecar/utterheim_sidecar/. " +
                "Check the build output of src/Utterheim/Utterheim.csproj.");
        }

        var destRoot = Path.Combine(
            _paths.PythonRuntimePath, "Lib", "site-packages", "utterheim_sidecar");
        ...
        _logger.LogInformation(
            "utterheim_sidecar copied from {Source} to {Dest}.", sourceRoot, destRoot);
    }

    /// <summary>
    /// Find the bundled package directory. In production it sits beside
    /// utterheim.exe under <c>PythonSidecar/utterheim_sidecar/</c>; in dev
    ...
    private static string? LocateBundledSidecarRoot()
    {
        var candidate = Path.Combine(AppContext.BaseDirectory,
            "PythonSidecar", "utterheim_sidecar");
        if (Directory.Exists(candidate)) return candidate;
        return null;
    }

    private bool UtterheimSidecarActuallyInstalled()
    {
        return File.Exists(Path.Combine(
            _paths.PythonRuntimePath, "Lib", "site-packages", "utterheim_sidecar", "__init__.py"))
            && File.Exists(Path.Combine(
                _paths.PythonRuntimePath, "Lib", "site-packages", "utterheim_sidecar", "main.py"));
    }

    /// <summary>
    /// True iff the installed <c>utterheim_sidecar/__init__.py</c>'s
    ...
    private bool BundledSidecarMatchesInstalled()
    {
        ...
        if (bundledRoot is null)
        {
            ...
            _logger.LogWarning(
                "Bundled utterheim_sidecar package not found next to utterheim.exe — forcing re-install attempt.");
            return false;
        }
        ...
        {
            _logger.LogWarning(
                "utterheim_sidecar __version__ unreadable (installed={Installed}, bundled={Bundled}) — forcing re-install.",
        ...
        {
            _logger.LogInformation(
                "utterheim_sidecar version drift: installed={Installed}, bundled={Bundled} — re-install will run.",
                installed, bundled);
            return false;
        }
...
public sealed class BootstrapState
{
    ...
    /// <summary>
    /// True after the bundled utterheim_sidecar wrapper has been copied into
    ...
    /// </summary>
    public bool UtterheimSidecarInstalled { get; set; }
