...
// Utterheim-flavoured: WhisperHeim-specific migration helpers stripped, paths
...
///
///   %APPDATA%\Utterheim\         bootstrap, settings, logs (synced-friendly)
///   <dataPath>\voices\             voice profiles + library.json
///   %LOCALAPPDATA%\Utterheim\    runtime\python, models, cache (machine-local)
///
/// dataPath defaults to %APPDATA%\Utterheim\ and can be redirected via bootstrap.json
...
/// temp+rename, and fires <see cref="DataPathChanged"/>. In Utterheim only the voice
...
public sealed class DataPathService
{
    ...
    public static readonly string RoamingRoot =
        Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Utterheim");

    /// <summary>Local-machine root: bundled Python, model weights, cache.</summary>
    public static readonly string LocalRoot =
        Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Utterheim");
